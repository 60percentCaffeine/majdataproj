...
namespace ModTestIntegration
{
    public sealed class SongFpsRegressionCanaryTests
    {
        ...
        public async Task KnownLocalChartGameplayFrameTimingStaysWithinConservativeThresholds()
        {
            ...
            try
            {
                ...
                Assert.True(setupState.Value.GetProperty("SettingsVSync").GetBoolean());
                Assert.True(setupState.Value.GetProperty("SettingsFullScreen").GetBoolean());
            ...
            finally
            {
                ...
                harness.CollectLogs("song-fps");
            }

            Exception logFailure = Record.Exception(() => IntegrationAssertions.AssertNoFatalBootFailures(Path.Combine(harness.ArtifactsRoot, "song-fps")));
    ...
    object displaySettings = settings.GetType().GetProperty(""Display"", InstanceFlags).GetValue(settings);
    object debugSettings = settings.GetType().GetProperty(""Debug"", InstanceFlags).GetValue(settings);
    ...
    return new {
        ...
        SettingsResolution = displaySettings.GetType().GetProperty(""Resolution"", InstanceFlags).GetValue(displaySettings).ToString(),
        SettingsFPSLimit = Convert.ToInt32(displaySettings.GetType().GetProperty(""FPSLimit"", InstanceFlags).GetValue(displaySettings)),
        SettingsVSync = Convert.ToBoolean(displaySettings.GetType().GetProperty(""VSync"", InstanceFlags).GetValue(displaySettings)),
        SettingsFullScreen = Convert.ToBoolean(debugSettings.GetType().GetProperty(""FullScreen"", InstanceFlags).GetValue(debugSettings)),
        ScreenWidth = UnityEngine.Screen.width,
        ScreenHeight = UnityEngine.Screen.height,
        ScreenFullScreen = UnityEngine.Screen.fullScreen,
