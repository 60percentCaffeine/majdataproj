...
- [x] `LevelBucketizer` maps integer, decimal, plus, blank, malformed, and boundary levels according to the PRD.
- [x] Difficulty count is based on nonempty usable levels and filters support `No`, `>1`, `>2`, and `>3`.
- [x] Difficulty grouping includes a song in every usable difficulty folder and places songs with no usable levels in `Other`.
- [x] Level grouping uses the selected difficulty's normalized level bucket.
- [x] Artist grouping maps blank/null artist to `Unknown Artist`.
- [x] Name grouping maps blank/null title to `Other`.
- [x] Rank grouping maps missing score to `No Play`.
- [x] Unit tests cover all grouping modes and default folder preservation.
